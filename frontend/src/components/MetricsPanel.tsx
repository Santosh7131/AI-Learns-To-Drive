...
function Stat({ label, value, accent }: { label: string; value: string; accent?: boolean }) {
  return (
    <div className="rounded-lg border bg-secondary/40 p-2.5">
      ...
      <div className={`num mt-0.5 text-sm font-semibold ${accent ? "text-brand" : "text-foreground"}`}>{value}</div>
    </div>
  );
}
...
function Sparkline({ data }: { data: number[] }) {
  if (data.length < 2) {
    return <div className="flex h-16 items-center justify-center text-[11px] text-muted-foreground">collecting data…</div>;
  }
  const w = 100, h = 40;
  ...
  return (
    <svg viewBox={`0 0 ${w} ${h}`} preserveAspectRatio="none" className="h-16 w-full">
      ...
      <polyline points={pts} fill="none" stroke="hsl(var(--brand))" strokeWidth="1.75" vectorEffect="non-scaling-stroke" strokeLinejoin="round" strokeLinecap="round" />
      ...
      <CardContent className="space-y-3">
        <div className="rounded-lg border bg-secondary/40 p-3">
          <div className="mb-1 flex items-baseline justify-between">
            <span className="text-[10px] uppercase tracking-wide text-muted-foreground">Mean return</span>
            <span className="num text-lg font-semibold text-brand">{m ? m.meanReturn.toFixed(2) : "—"}</span>
          </div>
        ...
        <div className="grid grid-cols-3 gap-2">
          ...
          <Stat label="Steps/s" value={m ? m.fps.toFixed(0) : "—"} accent />
          <Stat label="Best" value={m ? m.bestReturn.toFixed(1) : "—"} accent />
          <Stat label="Laps" value={m ? String(m.totalLaps) : "—"} />
